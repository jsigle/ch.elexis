...
public class ScriptView extends ViewPart {
	...
	private void makeActions(){
		...
		newScriptAction =
			new RestrictedAction(AccessControlDefaults.SCRIPT_EDIT, Messages
				...
				public void doRun(){
					InputDialog inp =
						new InputDialog(getSite().getShell(), Messages
							.getString("ScriptView.enterNameCaption"), //$NON-NLS-1$
		...
		execScriptAction =
			new RestrictedAction(AccessControlDefaults.SCRIPT_EXECUTE, Messages
				...
				public void doRun(){
					...
					if (sel != null && sel.size() != 0) {
						...
						try {
							...
							ExHandler.handle(ex);
							SWTHelper.showError("Fehler beim Ausführen des Scripts", ex
								.getMessage());
						}
					}
				}
