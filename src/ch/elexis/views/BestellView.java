...
public class BestellView extends ViewPart implements ISaveablePart2 {
	...
	class BestellungLabelProvider extends LabelProvider implements ITableLabelProvider {
		...
		public Image getColumnImage(final Object element, final int columnIndex){
			//System.out.print("jsdebug: BestellView.BestellungLabelProvider/Image 1\n");
			return null;
		}
		
		public String getColumnText(final Object element, final int columnIndex){
			//System.out.print("jsdebug: BestellView.BestellungLabelProvider/Text 1\n");
			if (element instanceof Bestellung.Item) {
				//System.out.print("jsdebug: BestellView.BestellungLabelProvider/Text 2\n");
				Item it = (Item) element;
				//System.out.print("jsdebug: BestellView.BestellungLabelProvider/Text 3\n");
				switch (columnIndex) {
				case 0:
					//System.out.print("jsdebug: BestellView.BestellungLabelProvider/Text 3.0\n");
					return Integer.toString(it.num);
				case 1:
					//System.out.print("jsdebug: BestellView.BestellungLabelProvider/Text 3.1\n");
					return it.art.getLabel();
				case 2:
					//System.out.print("jsdebug: BestellView.BestellungLabelProvider/Text 3.2\n");
					Kontakt k = it.art.getLieferant();
					//System.out.print("jsdebug: BestellView.BestellungLabelProvider/Text 3.21\n");
					return k.exists() ? k.getLabel() : Messages.getString("BestellView.Unknown"); //$NON-NLS-1$
				default:
					//System.out.print("jsdebug: BestellView.BestellungLabelProvider/Text 3.def\n");
					return "?"; //$NON-NLS-1$
				}
			}
			//System.out.print("jsdebug: BestellView.BestellungLabelProvider/Text 4\n");
			return "??"; //$NON-NLS-1$
		}
	...
	private void makeActions(){
		...
		loadAction = new Action(Messages.getString("BestellView.OpenOrder")) { //$NON-NLS-1$
				...
				public void run(){
					...
					//System.out.print("jsdebug: BestellView.OpenOrder 1\n");
					
					ListDialog dlg = new ListDialog(getViewSite().getShell());

					//System.out.print("jsdebug: BestellView.getViewSite()  "+getViewSite().toString()+"\n");
					//PartSite(id=ch.elexis.BestellenView,pluginId=ch.elexis,registeredName=Bestellung,hashCode=4848122)

					//System.out.print("jsdebug: BestellView.getViewSite().getShell()  "+getViewSite().getShell().toString()+"\n");
					//Shell {Elexis 2.1.6.dev -  js / jh  -  Kein Patient ausgewählt}
										
					//System.out.print("jsdebug: BestellView.OpenOrder 2\n");
					...
					//System.out.print("jsdebug: BestellView.OpenOrder 3\n");
					//System.out.print("jsdebug: BestellView.dlg "+dlg.toString()+"\n");
					//jsdebug: BestellView.dlg org.eclipse.ui.dialogs.ListDialog@12421db

					
					//System.out.print("jsdebug: BestellView.OpenOrder 3.0\n");
					
					if (dlg.open() == Dialog.OK) {
						//System.out.print("jsdebug: BestellView.OpenOrder 3.1\n");
						/*
						 * 201201260912js:
						 * If a user selects NO entry from the dialog to select an old order,
						 * and still clicks on OK, another error message dialog appears.
						 * The contents of that dialog are misleading, by the way:
						 * |ERROR| - Elexis startup: Nicht abgefangene EventLoop-Exception!0 [OK]
						 * Well, I guess that the brute access to dlg.getResult()[0]
						 * is a probable cause and add a condition check before
						 * (over-)using the (possibly empty) result from the dialog: 
						 */
						if (dlg.getResult().length>0) {		//js: Only a non-empty result should be used any further.
							Bestellung res = (Bestellung) dlg.getResult()[0];
							setBestellung(res);
						}									//js: No handling of else... required. It is ok for a user to select nothing and say ok to close the dialog.
						
						//System.out.print("jsdebug: BestellView.OpenOrder 4\n");
					}
				
				//System.out.print("jsdebug: BestellView.OpenOrder 5\n");
	...
	class BestellContentProvider implements IStructuredContentProvider {
		...
		public Object[] getElements(final Object inputElement){
			//System.out.print("jsdebug: BestellView.BestellContentProvider 1\n");
			Query<Bestellung> qbe = new Query<Bestellung>(Bestellung.class);
			//System.out.print("jsdebug: BestellView.BestellContentProvider 2\n");
