...
public class SSDRange {
	...
	public SSDRange(Element el){
		...
		if (v != null && v.matches("\\d+,\\d+,\\d+,\\d+")) {
			String[] koor = v.split(",");
			viewport =
				new Rectangle(Integer.parseInt(koor[0]), Integer.parseInt(koor[1]), Integer
					.parseInt(koor[2]), Integer.parseInt(koor[3]));
		}
		contents = el.getText();
	}
	...
	public Element toElement(){
		...
		setAttributeIfExists(el, ATTR_VIEWPORT, viewport == null ? null : StringTool
			.RectangleToString(viewport.x, viewport.y, viewport.width, viewport.height));
		if (contents != null) {
			el.setText(contents);
		}
		return el;
