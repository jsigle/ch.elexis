 ...
 *    
 * $Id: ImporterPage.java 6182 2010-03-01 18:30:24Z rgw_ch $
 *******************************************************************************/
	...
	public class ImporterJob extends Job {
		...
		protected IStatus run(final IProgressMonitor monitor){
			...
			try {
				...
				return new Status(Status.ERROR, Hub.PLUGIN_ID, Messages
					.getString("ImporterPage.importError")
	...
	static public class DBBasedImporter extends Composite {
		...
		public Result<JdbcLink> getConnection(){
			...
			if (h.results[0].equalsIgnoreCase("mysql")) { //$NON-NLS-1$
				...
				return new Result<JdbcLink>(Result.SEVERITY.ERROR, 1, Messages
					.getString("ImporterPage.unknownType"), null, true); //$NON-NLS-1$
			}
			if (ret != null) {
				try {
			...
			return new Result<JdbcLink>(Result.SEVERITY.ERROR, 2, Messages
				.getString("ImporterPage.couldntConnect"), ret, true); //$NON-NLS-1$
		}
	}
}
